...
def transform(channels, trans_args):
    if 'rotate' in trans_args:
        deg = uniform(*trans_args['rotate'])
    else:
        deg = 0
    flip_ver = choice([True, False])
    flip_hor = choice([True, False])
    for i in range(len(channels)):
        channels[i] = np.flipud(channels[i]) if flip_ver else channels[i]
        channels[i] = np.fliplr(channels[i]) if flip_hor else channels[i]
        channels[i] = rotate(channels[i], angle=deg)
    return channels

class DataRetriever(torch_data.Dataset):
    def __init__(self, paths, targets, train=False, img_size=(600, 600), trans_args=None):
    ...
    def __getitem__(self, index):
        ...
        for t in ("FLAIR", "T1w", "T1wCE"): # "T2w"  # Cycle through data types
            ...
            for i in r:  # Loop through images. Downsample, normalize, and append. 
                channel.append(cv2.resize(load_dicom(t_paths[i]), self.img_size) / 255)
            channel = np.mean(channel, axis=0)  # take average
            channels.append(channel)  # append average
        if self.train:
            channels = transform(channels, self.trans_args)
        y = torch.tensor(self.targets[index], dtype=torch.float)  # Targets to tensor
        # import pdb; pdb.set_trace()
        # return {"X": self.transform(np.array(channels)), 'y': y}
        return {"X": torch.tensor(channels).float(), "y": y}  # provide X and y

# Model extentends nn.Module
class Model(nn.Module):
    def __init__(self, model_num='7'):
        super().__init__()
        # use EfficientNet
        self.net = efficientnet_pytorch.EfficientNet.from_pretrained("efficientnet-b"+model_num)
        # Load weights
...
class Trainer:
    ...
    def fit(self, epochs, train_loader, valid_loader, save_path, patience, writer_name):
        # Write to tensorboard
        writer = SummaryWriter(log_dir='runs/{}'.format(writer_name))
...
if __name__=='__main__':
    ...
    parser.add_argument('--random-seed', dest='seed', type=int, help='An integer random seed', default=42)
    parser.add_argument('-e', '--epochs', dest='epochs', type=int, help='An integer number of epochs.', default=100000)
    parser.add_argument('-l', '--lr', dest='lr', type=float, help='A float for a learning rate.', default=0.001)
    parser.add_argument('-m','-model', dest='model', type=str, help='A model number from 0 to 7', default='7')
    parser.add_argument('-i', '--image-size', dest='img_size', type=int, help='An integer to be the image size.', default=224)
    parser.add_argument('--rotate-range', dest='rotation', type=float, help='A float to set the range of image rotation.', default=180.0)
    #TODO: Set flip hor arg
    #TODO: Set flip ver arg
    #TODO: Image noise
    #TODO: Image cropping
    #TODO: Image scaling
    #TODO: Pretrained Weights: yes or no
    #TODO: AddRayTune for parameter searching
    #TODO: Log parameters with tensorboard to compare results
    #TODO: Add pretrained tuning (unfreezing weights in phases)
    #TODO: Add early stopping and saving best models
    #TODO: Use OneFit Cycle
    ...
    trans_args = {'rotate':(-args.rotation, args.rotation)}
    ...
    model = Model(model_num=args.model)
    model.to(device)
    ...
    history = trainer.fit(
        ...
        train_loader, 
        valid_loader, 
        "best-model_{}.pth".format(dt_string), 
        100,
        writer_name=dt_string
    )